...
class FashionFlowState(Base):
    """Fashion flow state for tracking user's progress in fashion design flow."""
    ...
    current_step: Mapped[str] = mapped_column(
        String(50),
        default=FlowStep.IDLE,
    )
    fabric_url: Mapped[str | None] = mapped_column(
        String(500),
        default=None,
    )
    fabric_name: Mapped[str | None] = mapped_column(
        String(255),
        default=None,
    )
    client_measurements: Mapped[dict | None] = mapped_column(
        JSON,
        default=None,
    )
    garment_specs: Mapped[dict | None] = mapped_column(
        JSON,
        default=None,
    )
