...
class ProfileType:
    """Profile type values (stored as strings in PostgreSQL)."""
    CURIOSO = "curioso"
...
class User(Base):
    """User model for C&N Tecidos chatbot users."""
    ...
    remote_jid: Mapped[str] = mapped_column(
        String(255),
        unique=True,
        index=True,
    )
    profile_type: Mapped[str] = mapped_column(
        String(50),
        default=ProfileType.CURIOSO,
    )
    body_measurements: Mapped[dict | None] = mapped_column(
        JSON,
        default=None,
    )
    ...
    flow_state: Mapped["FashionFlowState | None"] = relationship(
        "FashionFlowState",
        back_populates="user",
        uselist=False,
        cascade="all, delete-orphan",
